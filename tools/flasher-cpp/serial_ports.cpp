...
static bool
is_tty_device(const std::string& name)
{
#ifdef _WIN32
...
#ifndef _WIN32
static std::string
read_sysfs_attr(const std::string& devname, const std::string& attr)
{
...
static std::string
resolve_product_name(const std::string& devname)
{
...
list_ports_win32()
{
  ...
  for (DWORD i = 0; SetupDiEnumDeviceInfo(dev_info, i, &dev_data); ++i) {
    ...
    if (RegQueryValueExA(hkey,
      ...
      if (is_tty_device(port_name)) {
        ...
        SetupDiGetDeviceRegistryPropertyA(dev_info,
                                          &dev_data,
                                          SPDRP_FRIENDLYNAME,
                                          nullptr,
        ...
        if (paren != std::string::npos)
          description = description.substr(0, paren);
  ...
  std::sort(
    ports.begin(), ports.end(), [](const PortInfo& a, const PortInfo& b) {
