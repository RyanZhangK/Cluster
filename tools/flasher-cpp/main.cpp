...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(600, 520, "Cluster Firmware Flasher",
                                          nullptr, nullptr);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (ImGui::Button("刷新")) {
            st.ports = list_serial_ports();
            if (!st.ports.empty() && st.selected_port < 0) st.selected_port = 0;
        }
        ImGui::SameLine();
        if (st.ports.empty()) {
        ...
        ImGui::InputText("##fw", st.firmware_path, sizeof(st.firmware_path),
                         ImGuiInputTextFlags_ReadOnly);
        ImGui::SameLine();
        if (ImGui::Button("浏览")) {
            std::string path = open_file_dialog();
            if (!path.empty()) {
                std::snprintf(st.firmware_path, sizeof(st.firmware_path), "%s",
                              path.c_str());
            }
        }
        ImGui::Spacing();
        ...
        if (ImGui::BeginCombo("##baud", baud_preview.c_str())) {
            for (int i = 0; i < static_cast<int>(st.baud_rates.size()); ++i) {
                ...
                if (ImGui::Selectable(label.c_str(), sel)) st.baud_index = i;
                if (sel) ImGui::SetItemDefaultFocus();
            }
            ImGui::EndCombo();
        }
